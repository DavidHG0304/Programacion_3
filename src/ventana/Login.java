...
//				System.out.println("Hola");
				...
				String pasarTexto = new String(cajaTexto2.getPassword());
				if(pasarTexto.length() <= 0) {
					cajaTexto2.setBorder(new LineBorder(Color.red, 4));
				}else {
					cajaTexto2.setBorder(new LineBorder(Color.GREEN, 4));
				}
				
				System.out.println(cajaTexto.getText());
				System.out.println(cajaTexto2.getPassword());
			}
		});
...
//		registro.add(bio);
		...
		botonR.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if(cajaTextoR.getText().length() <= 0) {
					cajaTextoR.setBorder(new LineBorder(Color.red, 4));
				}else {
					cajaTextoR.setBorder(new LineBorder(Color.green, 4));
				}
				if(cajaTextoR2.getText().length() <= 0) {
					cajaTextoR2.setBorder(new LineBorder(Color.red, 4));
				}else {
					cajaTextoR2.setBorder(new LineBorder(Color.green,4));
				}
				String pasarTexto = new String(cajaTextoR3.getPassword());
				if(pasarTexto.length() <= 0) {
					cajaTextoR3.setBorder(new LineBorder(Color.red,4));
				}else {
					cajaTextoR3.setBorder(new LineBorder(Color.green,4));
				}
				String pasarTexto2 = new String(cajaTextoR4.getPassword());
				if(pasarTexto2.length() <= 0) {
					cajaTextoR4.setBorder(new LineBorder(Color.red,4));
				}else {
					cajaTextoR4.setBorder(new LineBorder(Color.green,4));
				}
				int seleccionPais = paisRes.getSelectedIndex();
				if(paises[seleccionPais] == "Pais Residencia") {
					paisRes.setBorder(new LineBorder(Color.red,4));
				}else {
					paisRes.setBorder(new LineBorder(Color.green,4));
				}
				gusto1.setBorderPainted(true);
				if(!gusto1.isSelected()) {
...
//		calculadora.setBackground(Color.black);
		...
		JLabel input = new JLabel("20.0", 4);
		input.setOpaque(true);
		input.setBackground(Color.WHITE);
		input.setFont(new Font("Arial Black", Font.PLAIN, 40));
		calculadora.add(input, BorderLayout.NORTH);
		
		JPanel botonesNum = new JPanel();
		botonesNum.setLayout(new GridLayout(4,3,2,2));
//		botonesNum.setBackground(Color.lightGray);
		
		botonesNum.setBorder(BorderFactory.createEmptyBorder(0,0,0,20));
		calculadora.add(botonesNum, BorderLayout.CENTER);
		
		String botones [] = {"9","8","7","6","5","4","3","2","1","0","."};
		for (int i = 0; i < botones.length; i++) {
			botonesNum.add(new JButton(botones[i]));
			
		}
		
		JPanel botonesAct = new JPanel();
		botonesAct.setLayout(new GridLayout(6,1, 0,2));
		botonesAct.setBackground(Color.lightGray);
		calculadora.add(botonesAct, BorderLayout.EAST);
		
		String botones2 [] = {"+","-","*","/","=","CE"};
		for (int i = 0; i < botones2.length; i++) {
			botonesAct.add(new JButton(botones2[i]));
	...
	public void paint(Graphics g) {
		...
		int xS2[] = {200,100,30};
		int yS2[] = {100,30,100};
//		g2d.drawPolyline(xS2, yS2, 100);
		
		g2d.drawArc(250, 250, 250, 328, 192, 250);
		g2d.setColor(new Color(255,128,77));
		g2d.drawLine(100, 100, 200, 300);
		g2d.setColor(Color.red);
		g2d.drawRect(400, 100, 50, 70);
		g2d.setColor(new Color(200,50,0));
		g2d.drawOval(500, 550, 50, 80);
		g2d.setColor(Color.red);
		g2d.fillRect(400, 100, 50, 70);
		
		g2d.setColor(Color.green);
		g2d.drawPolygon(xS2, yS2, 3);
		g2d.fillPolygon(xS2, yS2, 3);
		g2d.setColor(Color.yellow);
		g2d.fillOval(500, 550, 50, 80);
